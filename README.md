...
⚠️⚠️ 本规则集 = 高强度 自用级验证， 实锤 = 高精度 + 99.99% 可用，如果你分流失败，建议用上述的 “ 超级省电clash配置模版 ” 进行二次验证，以便排除是否是您自己配置文件bug（尤其是引用rule规则时候 排序不当引发的bug）引发的问题。 
<br>
<br>
...
不会 ！ 这俩格式，纯属自嗨。 
 ...
 >
 > - mrs \ text 缺点 ：对 VPN 运行性能 0影响，且 无法添加注释，可维护性极差 。（后台静默更新规则集，是异步行为，用户 = 0感知，看不出优化价值何在）
