...
#	1. 苹果在绑定chatgpt操作过程中，是有LBS锁的。会通过GPS等位置来判定你是否在中国，如果在，则不会调用ChatGPT。只会调用文心一言。所以仅仅即便配置 本规则+登录Chat GPT，苹果AI也不会调用Chat GPT，而是会调用百度文心一言。
#	2.此时必须禁用siri的地理位置获取，才能让苹果AI调用ChatGPT。
